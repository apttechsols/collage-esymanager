<?php
	/*
	*@filename AptPdoFetchWithAes/index.php
	...
	*/
	...
	function AptPdoFetchWithAes($Data=array()){
		$DefaultCheckFor = 'All'; $DefaultCheckType = 'Equal'; $AcceptNullCondtion = False; $FetchRowNo = 1;
		foreach ($Data as $key=>$value){
			${ $key } = $value;
		}
		
		if($DbCon == '' || $TbName == '' || $EPass == '' || $DefaultCheckFor == '' || $DefaultCheckType == '' || $FetchRowNo == '' || ($AcceptNullCondtion != False && $AcceptNullCondtion != true)){
		...
		if($Condtion != ''){
			...
			foreach ($CondtionArray as $value){
				...
				if(strpos($value, "::::") !== False || (($TempCheckType == 'Equal' || $TempCheckType == 'NotEqual') && $AcceptNullCondtion == true)){
				   // Code
				} else{
		...
		if($FetchData != ''){
			...
			foreach ($FetchDataArray as $value) {
				...
				if(preg_replace("/[^A-Za-z0-9_]/","",$value) != "" && preg_replace("/[^A-Za-z0-9_]/","",$value) == $value){
					if($i == 1){
						$StmtFetchDataString ="AES_DECRYPT(".$value.", :EPass) AS ".$value;
					}else{
						$StmtFetchDataString = $StmtFetchDataString.", AES_DECRYPT(".$value.", :EPass) AS ".$value;
					}
				}else{
		...
		if($DefaultCheckType == 'ValLike'){
			#return_response($StmtString);
		}
