<?php
	...
	if($ResponseCheckServiceBuyStatus['status'] != 'Success' || $ResponseCheckServiceBuyStatus['code'] != 200){
		FullPageErrorMessageDisplay('We can not able to fetch service data due to technical error! If you already buy this service then please contact with customer care 1.3',true,['MSGpadding'=>'40vh 10px']); exit();
	}

	if($ResponseCheckServiceBuyStatus['msg']['ServiceBuy'] != True){
		FullPageErrorMessageDisplay('This service is not buyed by this orgaization! But if you buyed it then please contact with customer care 1.4',true,['MSGpadding'=>'40vh 10px']); exit();
	}
	...
	if($ResponseRank == 1 || $ResponseRank == 2){
		header("Location: " . RootPath . "Users/Service/Dashbord/D%20GatePass_a3cnvkaihl1580334506d13zswes11/Manager/index.php"); die();
	}

	if(isset($ServiceMemberData)){
		if($ServiceMemberData['status'] == "Error" && $ServiceMemberData['code'] == 404){
			FullPageErrorMessageDisplay('You are not a member of this service',true,['MSGpadding'=>'40vh 10px']); exit();
		}else if($ServiceMemberData['status'] != 'Success' || $ServiceMemberData['code'] != 200){
			FullPageErrorMessageDisplay('There is some technical error occur! Please contact with customer care 1.5',true,['MSGpadding'=>'40vh 10px']); exit();
		}else if($ServiceMemberData['msg']->Status != 'Active'){
			FullPageErrorMessageDisplay('You status is '.$ServiceMemberData['msg']->Status.' for this service',true,['MSGpadding'=>'40vh 10px']); exit();
		}
	}else{
		FullPageErrorMessageDisplay('There is some technical error occur! Please contact with customer care 1.6',true,['MSGpadding'=>'40vh 10px']); exit();
	}
	...
	if($LgUserServicePositionRank == 1){
		header("Location: " . RootPath . "Users/Service/Dashbord/D%20GatePass_a3cnvkaihl1580334506d13zswes11/Manager/index.php"); die();
	}else if($LgUserServicePositionRank == 2){
		header("Location: " . RootPath . "Users/Service/Dashbord/D%20GatePass_a3cnvkaihl1580334506d13zswes11/PermissionGiver/index.php"); die();
	}else if($LgUserServicePositionRank == 0){
		header("Location: " . RootPath . "Users/Service/Dashbord/D%20GatePass_a3cnvkaihl1580334506d13zswes11/User/index.php"); die();
	}else if($LgUserServicePositionRank == -1){
		header("Location: " . RootPath . "Users/Service/Dashbord/D%20GatePass_a3cnvkaihl1580334506d13zswes11/PermissionChecker/index.php"); die();
	}else{
		foreach (get_defined_vars() as $SetVarKey=>$SetVarVal){ unset($$SetVarKey); } unset($SetVarKey,$SetVarVal);
		FullPageErrorMessageDisplay('You are not a member of this service',true,['MSGpadding'=>'40vh 10px']); exit();
	}
