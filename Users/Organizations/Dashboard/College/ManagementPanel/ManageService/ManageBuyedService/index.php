<?php
	/*
	*@FileName ManageMembers/index.php
...
<?php
	...
	$GetError = array();
	$ResponseLogin = CkeckLogin($EncodeAndEncryptPass,$PdoMainUserAccountDb,$PdoOrganizationUserAccount,'Fullname::::Mobile::::UserUrl::::ProfileUrl::::Position');
	...
	foreach (get_defined_vars() as $SetVarKey=>$SetVarVal){ if($SetVarKey != 'RootPath' && $SetVarKey != 'ResponseLogin' && $SetVarKey != 'ResponseRank' && $SetVarKey != 'ResponseBuyedService' && $SetVarKey != 'ResponseServiceList' && $SetVarKey != 'GetError' && $SetVarKey != 'UserPartOfServiceStore' && $SetVarKey != '_GET' && $SetVarKey != '_POST' && $SetVarKey != '_SESSION' && $SetVarKey != '_COOKIE' && $SetVarKey != 'RandomPass1' && $SetVarKey != 'RandomPass2' && $SetVarKey != 'RandomPass3' && $SetVarKey != 'RandomPass4' && $SetVarKey != 'RandomPass5' && $SetVarKey != 'Token_CSRF'){ unset($$SetVarKey); }} unset($SetVarKey,$SetVarVal); ob_end_clean();
	...
	define("PageTitle", "YOUR SERVICES");
	define("CssFile", "ManageBuyedService");
	require_once RootPath."Site_Header/index.php";
?>
<body class="No_Select_Strat">
	<H1 class='PageTitle'>YOUR SERVICES</H1>
	<div class='Container' <?php if(sizeof($UserPartOfServiceStore) == 0 || sizeof($GetError) > 0 || sizeof($UserPartOfServiceStore) <= 1){ echo "style='grid-template-columns:1fr;'"; }?>>
		...
		<?php
			if(sizeof($UserPartOfServiceStore) == 0){
				FullPageErrorMessageDisplay('You are not a member of any service in this organization',true,['MSGpadding'=>'40vh 10px']); exit();
			}
