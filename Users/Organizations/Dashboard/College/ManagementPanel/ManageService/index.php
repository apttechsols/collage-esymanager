	...
	if($ResponseLogin['status'] != 'Success' || $ResponseLogin['code'] != 200){
		...
		if($ResponseLogin['LAS'] != 'OrganizationMember'){
			...
			if($ResponseRank != 1 && $ResponseRank != 2){
				header("Location: " . RootPath . "Users/Organizations/Dashboard/College/ManagementPanel/ManageService/ManageBuyedService/index.php"); die();
			}
		}
	}
	define("PageTitle", "Manage Service");
	define("CssFile", "ManageService");
	require_once RootPath."Site_Header/index.php";
?>
<body class="No_Select_Strat">
	<H1 class='PageTitle'>MANAGE SERVICE</H1>
	<div class='Container'>
		<div class='button BuyService'>
			<p>Buy Service</p>
		</div>
		<div class='button ManageService'>
			<p>Manage Service</p>
		</div>
	</div>
</body>
...
<script>
	$(document).ready(function(){
		$('.BuyService').click(()=>{
			window.location.href = '<?php echo RootPath; ?>Users/Service/Buy/Available/index.php';
		});
		$('.ManageService').click(()=>{
			window.location.href = 'ManageBuyedService/index.php';
		});
	});
</script>
</html>
