...
<?php
	...
	foreach (get_defined_vars() as $SetVarKey=>$SetVarVal){ if($SetVarKey != 'RootPath' && $SetVarKey != 'GetOrganizationData' && $SetVarKey != 'ResponseLogin' && $SetVarKey != 'ResponseRank' && $SetVarKey != '_GET' && $SetVarKey != '_POST' && $SetVarKey != '_SESSION' && $SetVarKey != '_COOKIE' && $SetVarKey != 'RandomPass1' && $SetVarKey != 'RandomPass2' && $SetVarKey != 'RandomPass3' && $SetVarKey != 'RandomPass4' && $SetVarKey != 'RandomPass5' && $SetVarKey != 'Token_CSRF'){ unset($$SetVarKey); }} unset($SetVarKey,$SetVarVal); ob_end_clean();// Unset all vars
	...
	if($ResponseLogin['status'] != 'Success' || $ResponseLogin['code'] != 200){
		...
		if($ResponseRank < $MemberOperationPermissionRankUpToAddMemberPermissionRankUpToStart || $ResponseRank > $MemberOperationPermissionRankUpToAddMemberPermissionRankUpToEnd){
    	    FullPageErrorMessageDisplay('You are not Authorized for Add Member',true,['MSGpadding'=>'40vh 10px']); exit();
    	}
	}
	define("PageTitle", "Add Member");
	define("CssFile", "AddNewMember");
		...
		<div class="SubContainer">
			<div class="ImageBox">
				<div class="ImageLabelBox">
					...
					<img src="<?php echo RootPath; ?>Image_store/cute_dog.jpg" class="ProfileImage" id="ProfileImage">
					<div>
			...
			<div class="Form1">
				<div class="FullNameBox">
					<input type="text" name="name" placeholder="Fullname" id="FullName" maxlength="30" spellcheck="false" autocomplete="none">
					<p id="FullNameError" class="Error"></p>
				</div>
				
				<div class="MobileNumberBox">
					<input type="text" name="number" placeholder="Mobile Number" id="MobileNumber" maxlength="15" autocomplete="none">
					<p id="MobileNumberError" class="Error"></p>
				</div>
				<div class="EmailBox">
					<input type="email" name="Email" class='Email' placeholder="Email (optional)" id="Email" spellcheck="false" autocomplete="none" maxlength="60">
				...
				<div class='OrgJoinTimeBox'>
					...
					<div class='OrgJoinTime'>
						<input type="number" class="OrgJoinDate" placeholder="Date" />
						<input type="number" class="OrgJoinMonth" placeholder="Month" />
						<input type="number" class="OrgJoinYear" placeholder="Year" />
					</div>
				...
				<div class="PrimaryBatchIdBox" hidden="true">
					<input type="text" class='SearchPrimaryBatchId' spellcheck="false" placeholder="Search Primary Batch Id">
				...
				<div class="SecondaryBatchIdBox" hidden="true">
					<input type="text" class='SearchSecondaryBatchId' spellcheck="false" placeholder="Search Primary Batch Id">
				...
				<div class="FathersNameBox" hidden="true">
					<input type="text" name="name" class='FathersName' placeholder="Father's Name" id="FathersName" maxlength="30" spellcheck="false" autocomplete="none">
					<p id="FathersNameError" class="Error"></p>
				</div>
				<div class="FathersMobileNumberBox" hidden="true">
					<input type="text" name="number" placeholder="Father's Mobile Number" id="FathersMobileNumber" maxlength="15" autocomplete="none">
					<p id="FathersMobileNumberError" class="Error"></p>
				</div>	
				<div class="FathersEmailBox" hidden="true">
					<input type="email" name="Email" class='FathersEmail' placeholder="Father's Email (optional)" id="FathersEmail" spellcheck="false" autocomplete="none" maxlength="60">
				...
				<div class="GuardianNameBox" hidden="true">
					<input type="text" name="name" class='GuardianName' placeholder="Guardian Name (optional)" id="GuardianName" maxlength="30" spellcheck="false" autocomplete="none">
					<p id="GuardianNameError" class="Error"></p>
				</div>
				<div class="GuardianMobileNumberBox" hidden="true">
					<input type="text" name="number" placeholder="Guardian Mobile Number (optional)" id="GuardianMobileNumber" maxlength="15" autocomplete="none">
					<p id="GuardianMobileNumberError" class="Error"></p>
				</div>	
				<div class="GuardianEmailBox" hidden="true">
					<input type="email" name="Email" class='GuardianEmail' placeholder="Guardian Email (optional)" id="GuardianEmail" spellcheck="false" autocomplete="none" maxlength="60">
				...
				<div class="StateBox">
					...
					maxlength="50" autocomplete="none">
				...
				<div class="AddressBox">
					<input class="Address" id="Address" placeholder="Address" maxlength="100" spellcheck="false" autocomplete="none"/>
					<p id="AddressError" class="Error"></p>
				</div>
				<div class="DemoBox" hidden="true">
					<input type="text" name="Demo" placeholder="Blank" id="Demo"
					maxlength="6" autocomplete="none" disabled="true">
					<p id="DemoError" class="Error"></p>
				</div>
...
<script>
	$(document).ready(function(){
		...
		/*// FathersEmail validations
		$("#FathersEmail").keyup(function(){
			var Email = $("#FathersEmail").val();
			if(Email.length > 0){
				var user_gmail_address_length = Email.substring(0,Email.length-10).length;
				var user_gmail_address_length_after_validations = Email.substring(0,Email.length-10).replace(/[^A-Za-z0-9.]/g,"").length;
				if(user_gmail_address_length == user_gmail_address_length_after_validations && user_gmail_address_length_after_validations >= 6 && user_gmail_address_length_after_validations <= 30){
					if(Email.substring(Email.length-10,Email.length) == "@gmail.com"){
						$("#FathersEmailError").html("");
					}
				}
			}else{
				$("#FathersEmailError").html("");
		...
		$("#FathersEmail").blur(function(){
			...
			if(Email.length > 0){
				...
				if(user_gmail_address_length == user_gmail_address_length_after_validations && user_gmail_address_length_after_validations >= 6 && user_gmail_address_length_after_validations <= 30){
					if(Email.substring(Email.length-10,Email.length) != "@gmail.com"){
						$("#FathersEmailError").html("This gmail is invalid");
					}
				}else{
					$("#FathersEmailError").html("This gmail is invalid");
				}
		...
		$("#Position").change(function() {
				...
				if(this.value === 'Student'){
					...
					$(".FathersNameBox, .FathersMobileNumberBox, .FathersEmailBox, .BranchBox, .YearBox, .SemesterBox, .SecondaryBatchIdBox ,.PrimaryBatchIdBox,.GuardianGenderBox,.GuardianNameBox,.GuardianMobileNumberBox,.GuardianEmailBox,.DemoBox").prop('hidden',false);
					$("#UniqueId").attr('placeholder','Unique Id');
				}else{
					$(".FathersNameBox, .FathersMobileNumberBox, .FathersEmailBox, .BranchBox, .YearBox, .SemesterBox, .SecondaryBatchIdBox ,.PrimaryBatchIdBox,.GuardianGenderBox,.GuardianNameBox,.GuardianMobileNumberBox,.GuardianEmailBox,.DemoBox").prop('hidden',true);
		...
		$("#SubmitButton").click(function(){
			...
			if(window.SubmitError == true){
				...
				if(document.getElementById("ProfileImageFile").files.length > 1){
					...
					return false;
				}else if(document.getElementById("ProfileImageFile").files.length == 0){
					SubmitReset();
					swal("Please Select Profile Image")
					.then((value) => {
						$(".ProfileImageFileLabel").click();
					});
					return false;
			...
			imprint.test(browserTests).then(function(result){
				...
				audioFingerprint.run(function (fingerprint_2) {
					...
					try{
						...
						ajax.open("POST", "AddNewMemberBackend.php");
						ajax.send(formdata);
					}catch(error){
			...
			{	
				...
				if(response['status'] === "Success" && response['code'] === 200){
					...
					.then(() => {
						...
						$("#GuardianGender").val('');
						$("#FathersName").val('');
						$("#FathersMobileNumber").val('');
						$("#FathersEmail").val('');
						$("#GuardianName").val('');
						$("#GuardianMobileNumber").val('');
						$("#GuardianEmail").val('');
						$("#ProfileImageFile").val('');
						$("#ProfileImage").attr('src',RootPath+"Image_store/cute_dog.jpg");
						return false;
					});
				}else{
