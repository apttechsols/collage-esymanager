...
<?php
	...
	define("PageTitle", "Search Service");
	define("CssFile", "SearchService");
...
?>
	<body>
		<div class = "MainContainer">
			...
			<div class="SearchResultBox">
				<div class = "NewResponseMainBox">Services Appear Here</div>
			</div>
		</div>
	</body>
	...
	<script>
				...
				imprint.test(browserTests).then(function(result){
					...
					audioFingerprint.run(function (fingerprint_2) {
						...
						try{
							...
							ajax.open("POST", "SearchServiceBackend.php");
							ajax.send(formdata);
						}catch(error){
						...
						function ResponceHandler(event)
						{
							...
							if(responce['status'] == "Success" && responce['code'] == "200")
							{
								for(var i = 0; i < responce['msg']['totalrows']; i++){
									$('.SearchResultBox').append('<div class="NewSearchResponseContainer'+i+'"><div class = "NewResponseMainBox" style="grid-template-columns : repeat(6,1fr); padding-top : 0px;"><div class="StatusBox"><p>Status</p><p>'+responce['msg']['msg'][i]['Status']+'</p></div><div class="NameBox"><p>Name</p><p>'+responce['msg']['msg'][i]['Name']+'</p></div><div class="StartTimeBox"><p>Start Time</p><p>'+responce['msg']['msg'][i]['StartTime']+'</p></div><div class="ExpTimeBox"><p>Expire Time</p><p>'+responce['msg']['msg'][i]['ExpTime']+'</p></div><div class="LastUpdate"><p>Last Update</p><p>'+responce['msg']['msg'][i]['LastUpdateTime']+'</p></div><div class="TotalSelledPackBox"><p>Total Selled Pack</p><p>'+responce['msg']['msg'][i]['TotalSelledPack']+'</p></div><div class="CodeBox" hidden><p>Code</p><p>'+responce['msg']['msg'][i]['Code']+'</p></div><div class="SearchDataKeyBox" hidden><p>SearchDataKeyBox</p><p>'+SearchDataKey+'</p></div></div><div class="SearchResultButtonBox"><div class="ViewServices" onclick="ViewServices('+i+')">View</div><div class="ShowPlans" onclick="ShowPlans('+i+')">Show Plans</div><div class="UpdateServices" onclick="UpdateServices('+i+')">Update</div><div class="DeleteServices" onclick="DeleteServices('+i+')">Delete</div></div></div>');
								}
							}
							else
							{
		...
		function ViewServices(getData){
			window.location.href = "../ViewService/index.php?target="+$('.NewSearchResponseContainer'+getData+' .NewResponseMainBox .CodeBox p:eq(1)').html();

		}

		function ShowPlans(getData){
			window.location.href = "../SearchPlan/index.php?target="+$('.NewSearchResponseContainer'+getData+' .NewResponseMainBox .NameBox p:eq(1)').html();
		
		}

		function UpdateServices(getData){
			window.location.href = "../UpdateService/index.php?target="+$('.NewSearchResponseContainer'+getData+' .NewResponseMainBox .CodeBox p:eq(1)').html();

		}

		function DeleteServices(getData){
			...
			audioFingerprint.run(function (fingerprint_2) {
				...
				swal("Are you sure you want to delete this service? Beause this action can not be undo", {
					buttons: {
						cancel: "Cancel",
						Next: true,
					},
						...
						.then((value) => {
							if(value === "Next"){
								if($(".CheckResult").val() == TempNum1+TempNum2){
									swal('Enter your Security Code to Delete this service', {
										buttons: {
											cancel: "Cancel",
											Next: true,
										},
									...
									.then((value) => {
										if(value === "Next"){
											...
											var formdata = new FormData();
											formdata.append('target', "<?php echo $_GET['target']; ?>");
											formdata.append("Code", $('.NewSearchResponseContainer'+getData+' .NewResponseMainBox .CodeBox p:eq(1)').html());
											...
											try{
												var ajax = new XMLHttpRequest();
												ajax.addEventListener("load",ResponceHandler,false);
												ajax.open("POST", "../DeleteService/index.php");
												ajax.send(formdata);
											}catch(error){
												swal(error,'','error');
												return false;
											...
											function ResponceHandler(event){
												...
												if(responce['status'] == "Success" && responce['code'] == "200"){
													...
													.then(()=>{
														window.location.replace("<?php echo RootPath; ?>Users/Main/Dashboard/ManageServices/SearchService/index.php?target="+$('.NewSearchResponseContainer'+getData+' .NewResponseMainBox .SearchDataKeyBox p:eq(1)').html()); return false;
													});	
												}else{
